...
unsafe fn u5_to_u8_unsafe(x: u8) -> u8 {
    ...
    unsafe {
        ...
        //         Since `to_int_unchecked` truncates, the integer value of f
        //         will be between in-range for u8.
        f.to_int_unchecked()
    }
}
```
...
- The generated C code may not be standard conforming, due to the lack of a standardized 128-bit integer type. If the code uses the <code class="language-c">uint128_t</code> type, replace it with the appropriate 128-bit integer type for your compiler. See [this Stack Overflow answer](https://stackoverflow.com/a/54815033/7595472) for more details.
