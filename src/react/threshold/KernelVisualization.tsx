...
function VInput({
    ...
    return (
        <label htmlFor={id} className="flex flex-wrap items-center sm:flex-nowrap">
            <span className="-mb-2 mr-1 w-full whitespace-nowrap font-serif italic sm:mb-0 sm:w-14 sm:text-right">
                {label + " = "}
            </span>
            <NumberInput
                ...
                {labels.map((label, index) => {
                    ...
                    return (
                        <tr
                            key={index}
                            className={
                                "leading-snug cursor-pointer hover:bg-black relative text-sm xs:[font-size:inherit] mb-1 sm:mb-0 " +
                                (selected ? "bg-black text-white font-bold outline-1" : "")
                            }
